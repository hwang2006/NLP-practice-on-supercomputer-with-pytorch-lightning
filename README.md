...
## Motivations
NLPBOOK is designed for the NLP fine-tuning tasks on Google Colab, but I find it slow and somewhat inconvenient, for example, with the need to mount Google Drive every time. Therefore, I decided to accelerate the training practices by utilizing a supercomputer. I had to make some modifications to the original code to enable the execution of fine-tuning tasks from pre-trained models such as BERT and GPT-2 on a KISTI GPU cluster with multiple GPU nodes.
...
source ~/.bashrc
conda activate nlp
cd /scratch/$USER/nlp     # the root/work directory of the jupyter lab/notebook to be launched
...
(nlp) [gpu32]$ srun -N 2 --ntasks-per-node=2 python NLP-practice-on-supercomputer-with-pytorch-lightning/src/ner-train.py --num_nodes 2
(nlp) [gpu32]$ srun -N 2 --ntasks-per-node=2 python NLP-practice-on-supercomputer-with-pytorch-lightning/src/QA-train.py --num_nodes 2
...
(nlp) [gpu32]$ srun -N 2 --ntasks-per-node=2 python NLP-practice-on-supercomputer-with-pytorch-lightning/src/ner-train.py --num_nodes 2 --devices 2
(nlp) [gpu32]$ srun -N 2 --ntasks-per-node=2 python NLP-practice-on-supercomputer-with-pytorch-lightning/src/QA-train.py --num_nodes 2 --devices 2
...
## Running a Flask web server for exercising inference with the fine-tuned models
...
1. create a batch script for launching a Flask web server that will load a fine-tuned model that was saved when being trained:
```
...
```
4. check the SSH port forwarding information generated by the flask_run.sh script:
```
[glogin01]$ cat flask_port_forwarding_command
ssh -L localhost:5000:gpu##:##### $USER@neuron.ksc.re.kr
```
5. open a SSH client (e.g., Putty, PowerShell, Command Prompt, etc) on your PC or laptop and log in to Neuron just by copying and pasting the flask_port_forwarding_command:
```
C:\Users\hwang>ssh -L localhost:8888:gpu##:##### $USER@neuron.ksc.re.kr
Password(OTP):
Password:
```
6. open a web browser on your PC or laptop to access the Flask web server:
```
URL Address: localhost:5000
```
<p align="center"><img src="https://github.com/hwang2006/KISTI-DL-tutorial-using-horovod/assets/84169368/01fae688-e2c0-4dcc-9c4c-094d06654849" width="55%" height="55%"/></p> 
